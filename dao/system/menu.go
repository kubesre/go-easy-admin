...
type InterfaceMenus interface {
	AddMens(menu *system.Menu) error
	MenusList() ([]system.Menu, error)
}
...
func (m *menusInfo) AddMens(menu *system.Menu) error {
	err := global.GORM.Model(&system.Menu{}).Create(&menu).Error
	return err
}

// 菜单列表

func (m *menusInfo) MenusList() ([]system.Menu, error) {
...
func childrenMenu(menu *system.Menu) error {
